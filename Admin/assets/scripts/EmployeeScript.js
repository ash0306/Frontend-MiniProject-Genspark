...
function addRow(element) {
  ...
  row.innerHTML = `
        ...
        <td>
            ...
            <input type="number" class="salary-edit form-control d-none" value="${
              element.salary
            }">
        ...
        <button class="${buttonClass}" onclick="${functionName}('${
    element.id
  }')">${buttonText}</button>
        </td>`;
    ...
    .then(async (response) => {
      ...
      if (response.ok) {
        userDiv.innerHTML = `
            ...
            <li class="list-group-item"><strong>Date Of Birth : </strong>${new Date(
              data.dateOfBirth
...
function deactivateEmployee(employeeId) {
  ...
  fetch(
    "http://localhost:5228/api/employee/deactivateEmployee?id=" + employeeId,
    {
      method: "PUT",
      headers: {
...
function activateEmployee(employeeId) {
  ...
  fetch(
    "http://localhost:5228/api/employee/activateEmployee?id=" + employeeId,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
